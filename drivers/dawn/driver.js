...
const DAWNDevice = require('./device'); // Se till att rätt klass importeras om den inte redan är det

class DAWNDriver extends Driver {

  async onInit() {
    try {
      this.log('DAWNDriver has been initialized');
      this.log(`Homey version: ${this.homey.version}`);
    } catch (error) {
      this.error('Error initializing DAWNDriver:', error.message, error.stack);
    }
  }

  /**
   * List available devices for pairing
   * This method fetches all owned Charge Amps devices using the API and returns only DAWN devices for pairing in Homey.
   */
  async onPairListDevices() {
    try {
      ...
      // Filter devices to only include those of type "DAWN"
      const dawnDevices = response.data.filter(device => device.type === 'DAWN');

      if (dawnDevices.length === 0) {
        this.log('No DAWN devices found.');
        return [];
      }

      // Map the filtered DAWN devices to Homey compatible format for pairing
      const devices = dawnDevices.map((device, index) => {
        this.log(`Processing DAWN device ${index + 1}: ID = ${device.id}, Name = ${device.name}`);
      ...
      this.log(`DAWN devices ready for pairing: ${JSON.stringify(devices)}`);
      return devices;
    } catch (error) {
  ...
  /**
   * Create a new DAWNDevice instance
  ...
  createDevice(deviceData) {
    try {
      ...
      // Create the device
      const device = new DAWNDevice(this, deviceData);
